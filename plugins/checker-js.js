...
let getLongestExpr = (node, varTracker) => {
...
module.exports = {
    getWalker(tmpl, e) {
        ...
        let processService = node => {
            ...
            let tmplLines = tmpl.split(returnNewlineReg);
            ...
            let assign = (expr, oexpr, vname, sge) => {
                if (expr.type == 'MemberExpression' ||
                    ...
                    //获取哪个包含指向服务端的数据且是最长的
                    let c = getLongestExpr(expr.consequent, varTracker);
                    let a = getLongestExpr(expr.alternate, varTracker);
                    let aexpr;
                    /*
                        var a=list?list.d:[];
                     */
                    ...
                    if (expr.type == 'CallExpression') {
                        ...
                        if (isBC) {
                            ...
                            if (isMissingDefaultValueBagCall(expr)) {
                                let lines = tmpl.slice(0, expr.start).split(returnNewlineReg);
                                let line = (lines[lines.length - 1] + tmpl.slice(expr.start, expr.end)).trim();
                                let replacement = line.slice(0, -1) + ',defaultValue)';
                                maybeError.push({
                                    type: 'bc',
                                    pos: expr.start,
                                    line,
                            ...
                            if (info.key) {
                                ...
                                if (start.name) {
                                    ...
                                    if (p) {
                                        ...
                                        //console.log('0000', start.name, varTracker);
                                        memberExpressions.push({
                                            values: values,
                                            start: expr.start,
                                            end: expr.end
            ...
            if (Object.keys(paramsObject).length == Object.keys(usedParams).length) {
                memberExpressions.forEach(it => {
                    if (it.type == 'bc') {
                        maybeError.push({
                            pos: it.start,
                            near: tmplLines[tmpl.slice(0, it.start).split(returnNewlineReg).length - 1].trim(),
                            msg: tmpl.slice(it.start, it.end)
                        });
                    } else {
                        let p = getNearestParent(it);
                        if (p) {
                            let i = 1;
                ...
                maybeError.sort((a, b) => {
                    ...
                    if (it.type == 'bc') {
                        slog.ever(('avoid use: ' + it.line).red, 'at', e.shortFrom.gray, 'use', it.replacement.red, 'instead');
                    } else {
                        slog.ever(('may trigger an error: ' + it.msg).red, 'at', e.shortFrom.gray, 'near', it.near.magenta);
                    }
                });
        ...
        return {
            FunctionDeclaration: processService,
            FunctionExpression: processService,
            ArrowFunctionExpression: processService
        };
    }
};