...
module.exports = function(text, len, configKey) {
    if (md5Cache[text]) return md5Cache[text];
    //var buf = new Buffer(text);
    //var str = buf.toString('binary');
    var str = crypto.createHash('sha512').update(text, 'ascii').digest('hex');
    ...
    if (md5Cache[md5ResultKey + rstr] == 1) {
        ...
        var msg = 'generate "' + text + '" duplicate md5 result,please update config ' + configKey;
        if (configs.log) {
            console.log(msg.red);
        }
        throw new Error(msg);
